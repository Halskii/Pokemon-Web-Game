...
public class BattleController {
    ...
    public ResponseEntity<Battle> startBattle(@RequestBody Map<String, Integer> request) {
        ...
        try {
            Battle battle = battleService.startBattle(playerPokemonId);
    ...
    @PostMapping("/{battleId}/item")
    public ResponseEntity<Battle> useItem(
            @PathVariable String battleId,
        ...
        try {
            Battle battle = battleService.useItem(battleId, itemId);
            return ResponseEntity.ok(battle);
        } catch (IllegalArgumentException | IllegalStateException e) {
            return ResponseEntity.badRequest().build();
        }
